...
module VGA_CTRL_homework (
    ...
    always @(posedge CLK_33M or posedge Reset_p)
        if (Reset_p) vcount <= 0;
        else if ((vcnt >= VCNT_TOP_BORDER) && (vcnt < VCNT_DATA)) vcount <= vcnt - VCNT_TOP_BORDER;
        else vcount <= 0;

    always @(posedge CLK_33M or posedge Reset_p)
        if (Reset_p) VGA_HS <= 0;
        else if (hcnt == HCNT_SYNC) VGA_HS <= 1;
        else if (hcnt == 0) VGA_HS <= 0;
    always @(posedge CLK_33M or posedge Reset_p)
        if (Reset_p) VGA_VS <= 0;
        else if (vcnt == VCNT_SYNC) VGA_VS <= 1;
        else if (vcnt == 0) VGA_VS <= 0;
    always @(posedge CLK_33M or posedge Reset_p)
        if (Reset_p) VGA_DATA <= 0;
