module VGA_CTRL (
    ...
    output VGA_BLK;
    output VGA_CLK;
    output [11:0] VGA_DATA;  // for RGB444
    ...
    parameter VCNT_SYNC = 4;  // VSync_Time
    parameter VCNT_TOP_BORDER = VCNT_SYNC + 23 + 0;
    parameter VCNT_DATA = VCNT_TOP_BORDER + 600;
    parameter VCNT_FRONT_PORCH = VCNT_DATA + 0 + 1 - 1;
    always @(posedge CLK or posedge Reset_p)
        if (Reset_p) hcnt <= 0;
        else if (hcnt == HCNT_FRONT_PORCH) hcnt <= 0;
        else hcnt <= hcnt + 1'b1;
    always @(posedge CLK or posedge Reset_p)
        if (Reset_p) vcnt <= 0;
        else if (hcnt == HCNT_FRONT_PORCH)
            if (vcnt == VCNT_FRONT_PORCH) vcnt <= 0;
            else vcnt <= vcnt + 1'd1;
        else vcnt <= vcnt;
    // 有效數據輸出範圍
    assign VGA_BLK=
